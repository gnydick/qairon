...
resource "aws_internet_gateway" "public" {
  ...
  tags = "${merge(
                    map("Region", "${var.region}"),
                    map("Environment", "${var.environment}"),
                    map("Name", "${local.prefix}.public.internet_gateway"),
                    map("Config","${var.config_name}"),
                    map("GeneratedBy", "terraform"),
                    map("Tier", "public"),
                    var.extra_tags["public"])}"
...
resource "aws_route_table" "public" {
  ...
  tags = "${merge(
                    map("Region", "${var.region}"),
                    map("Environment", "${var.environment}"),
                    map("Name", "${local.prefix}.public.route_table"),
                    map("Config","${var.config_name}"),
                    map("GeneratedBy", "terraform"),
                    map("Tier", "public"),
                    var.extra_tags["public"])}"
...
resource "aws_route_table" "private" {
  ...
  tags =    "${merge(
                    map("Region", "${var.region}"),
                    map("Environment", "${var.environment}"),
                    map("Name","${local.prefix}.private.route_table"),
                    map("Config","${var.config_name}"),
                    map("GeneratedBy", "terraform"),
                    map("Tier", "private"),
