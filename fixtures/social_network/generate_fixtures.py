...
def generate(txt_output_dir=None, cloud_providers=None, include_apps=None):
    """Generate all fixture data in dependency order."""
    if cloud_providers is None:
        cloud_providers = ['aws']

    # Determine active applications
    active_applications = list(APPLICATIONS)
    if include_apps:
        for app in include_apps:
            if app in OPTIONAL_APPLICATIONS and app not in active_applications:
    ...
    for app, stacks in STACKS.items():
        if app not in active_applications:
            continue
    ...
    for stack_id, services in SERVICES.items():
        if stack_id.split(':')[0] not in active_applications:
            continue
...
def main():
    ...
    parser.add_argument('--include-infra', action='store_true',
                        help='Include the infra application (observability, cicd, security, networking, data stacks)')
    ...
    with app.app_context():
        ...
        if not args.monitoring_fixtures:
            include_apps = ['infra'] if args.include_infra else []
            print(f"\nCloud providers: {', '.join(args.providers)}")
            if include_apps:
                print(f"Optional applications: {', '.join(include_apps)}")
            print("Generating fixtures...")
            generate(txt_output_dir=args.txt_output, cloud_providers=args.providers, include_apps=include_apps)
