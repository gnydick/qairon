...
* RBAC Rules for Jenkins master.
* Persistent Volume on 32GB for storing Jenkins home.
* All using Kubernetes Deployments

## Deploy Jenkins Helm Chart

Make sure that you in `./helm-jenkins` directory!
```bash
$ pwd
/your/pc/path/to/repo/env-cicd/helm-jenkins
```
To deploy helm chart chart with the release name `jenkins`:
```bash
$ helm install --name jenkins --namespace jenkins -f values.yaml stable/jenkins
```
For checking Jenkins startup you can run:
```bash
$ kubectl logs -f <jenkins_pod_ip> -n jenkins
```
And check created Jenkins parts by:
```bash
$ kubectl get all -n jenkins
```
Now we need create Route53 DNS record for Jenkins domain:
```bash
$ aws route53 change-resource-record-sets --hosted-zone-id ZNCVYHIJBFV1L --change-batch file://create_dns.json
```
Last step will be deploy K8s Ingress Rule:
```bash
$ kubectl apply -f ./jenkins-ingress.yaml
```
Check K8s Ingress Rule:
```bash
$ kubectl get ing -n jenkins
```
Now you can check it from your browser:

* https://jenkins.withme.com

## Update Jenkins by Helm

Write changes in `values.yaml` (add plugin or change it version or change Jenkins image etc.)
```bash
$ helm upgrade jenkins stable/jenkins -f values.yaml
```
Restart and update can take 3-7 min

Check status of Jenkins master pod by command:
```bash
$ kubectl get po -n jenkins | grep "jenkins-*"
```
Check startup logs of Jenkins master pod by command:
```bash
$ kubectl logs -f $(kubectl get po -n jenkins | grep "jenkins-*" | awk -F ' ' '{print $1;}') -n jenkins
```
...
jenkins	7       	Thu Apr 18 16:56:25 2019	DEPLOYED	jenkins-0.36.2	lts        	jenkins
```
