...
# the withme.com domain (e.g. int-1.withme.com) to an AWS load balancer created from a
...
function help() {
  ...
  echo
  echo " -s <arg>"
  echo " --subdomain <arg>    The sub-domain for the environment (e.g. int-1.withme.com)"
  echo
  echo " -a <arg"
...
if [ -z "${LOAD_BALANCER_HOSTED_ZONE}" ]; then
