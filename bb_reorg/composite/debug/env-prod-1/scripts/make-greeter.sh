#!/bin/bash
#
# Searches for a username matching the provided argument exactly. If it is found, then it updates the account
# document for that user to put a "[Greeter]" prefix in front of the user's name.
#
# Author: Erik K. Worth
#
...
use account
db.account.find({"profile.username":"${USERNAME}"},{"profile.username":1})
EOF
...
if [ -z "${ACCOUNT_ID}" ]; then
  echo "No account matches username: ${USERNAME}"
  exit 1
fi

echo "Renaming username from '${USERNAME}' to '[Greeter] ${USERNAME}'"
...
use account
db.account.update({"_id" : ObjectId("${ACCOUNT_ID}")},{\$set: {"profile.username": "[Greeter] ${USERNAME}"}});
EOF
