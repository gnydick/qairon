...
./qcli pop_type create aws # pop type is like cloud vendor
./qcli pop_type create dev # dev as a pop type means somewhere in our dev env, could be laptop
./qcli pop create aws 126252960572  # creates aws account
./qcli pop create dev laptop # creates a dev pop to represent laptop
./qcli region create aws:126252960572 us-west-2 # self explanatory
./qcli region create dev:laptop here # just need a place holder region for local development
./qcli zone create aws:126252960572:us-west-2 usw2-az1 # availability zones
...
./qcli partition create aws:126252960572:us-west-2 vpc0 -n vpc-074b080f312234a06 # partition is a VPC or the like in other venders
./qcli partition create dev:laptop:here default  # again, placeholder for local development
./qcli network create aws:126252960572:us-west-2:vpc0 default 10.0.0.0/16 # records the cidr for our vpc
./qcli subnet create aws:126252960572:us-west-2:vpc0:default eks_nodes1 "10.0.5.0/24" -n "subnet-053d6f76c4be72b99" # recording all of the subnets that have already been created
./qcli subnet create aws:126252960572:us-west-2:vpc0:default eks_cp3 "10.0.0.48/28" -n "subnet-0166277520e0ac30a" # normally, qairon will create these entries and tell you the x.x.x.x/y
...
EOF
  ...
  # Configuration data for services and deployments
